...
// nodes have some means of representing cycles e.g., a linked list
//
// class List{
//   List *next;
//   List *prev;
...
inline auto strongConnect(G g, State<vertex_t<G>> state,
                          vertex_t<G> *v) -> State<vertex_t<G>> {
...
inline auto stronglyConnectedComponents(G g,
                                        vertex_t<G> *seed) -> vertex_t<G> * {
  using N = vertex_t<G>;
  State<N> state{};
  for (auto *v : g.getVertices(seed))
    if (!v->visited()) state = strongConnect(g, state, v);
...
inline auto topVisit(G g, N *list, N *v) -> N * {
  v->visit();
  for (auto *w : g.outNeighbors(v))
    if (!w->visited()) list = topVisit(g, list, w);
...
inline auto topSort(G g, vertex_t<G> *seed) -> vertex_t<G> * {
  ...
  N *list{nullptr};
  for (auto *v : g.getVertices(seed))
    if (!v->visited()) list = topVisit(g, list, v);
  return list;
}
