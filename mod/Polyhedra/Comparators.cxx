...
using math::PtrVector, math::MutPtrVector, math::Vector, math::_, math::Row,
  ...
  math::StridedVector, math::vector, math::square_matrix, math::identity,
  math::Simplex, math::DenseDims, math::DenseMatrix, math::vector, math::matrix;
...
struct EmptyComparator {
  ...
  static constexpr auto greaterEqual(PtrVector<int64_t>,
                                     PtrVector<int64_t>) -> bool {
    return true;
  }
  static constexpr auto greater(PtrVector<int64_t>,
                                PtrVector<int64_t>) -> bool {
    return false;
  }
  static constexpr auto lessEqual(PtrVector<int64_t>,
                                  PtrVector<int64_t>) -> bool {
    return true;
  }
  ...
  static constexpr auto equalNegative(PtrVector<int64_t>,
                                      PtrVector<int64_t>) -> bool {
    return true;
  }
...
struct LiteralComparator {
  ...
  static constexpr auto greaterEqual(PtrVector<int64_t> x,
                                     PtrVector<int64_t> y) -> bool {
    return x[0] >= y[0];
  }
  static constexpr auto greater(PtrVector<int64_t> x,
                                PtrVector<int64_t> y) -> bool {
    return x[0] > y[0];
  }
  static constexpr auto lessEqual(PtrVector<int64_t> x,
                                  PtrVector<int64_t> y) -> bool {
    return x[0] <= y[0];
  }
  static constexpr auto less(PtrVector<int64_t> x,
                             PtrVector<int64_t> y) -> bool {
    return x[0] < y[0];
  }
  static constexpr auto equal(PtrVector<int64_t> x,
                              PtrVector<int64_t> y) -> bool {
    return x[0] == y[0];
  }
...
template <typename T> struct BaseComparator {
  ...
  greaterEqual(MutPtrVector<int64_t> delta, PtrVector<int64_t> x,
  ...
  [[nodiscard]] constexpr auto
  greaterEqual(PtrVector<int64_t> x, PtrVector<int64_t> y) const -> bool {
  ...
  [[nodiscard]] constexpr auto greater(PtrVector<int64_t> x,
    ...
    invariant(N <= x.size());
    invariant(N <= y.size());
    Vector<int64_t> delta(math::length(N));
    for (ptrdiff_t n = 0; n < N; ++n) delta[n] = x[n] - y[n];
    --delta[0];
    return static_cast<const T *>(this)->greaterEqual(delta);
  }
  ...
  [[nodiscard]] constexpr auto
  greaterEqual(PtrVector<int64_t> x) const -> bool {
    return static_cast<const T *>(this)->greaterEqual(x);
  }
  [[nodiscard]] constexpr auto
  lessEqual(MutPtrVector<int64_t> x) const -> bool {
    const ptrdiff_t N = getNumConstTerms();
    invariant(N <= x.size());
  ...
  [[nodiscard]] constexpr auto
  equalNegative(PtrVector<int64_t> x, PtrVector<int64_t> y) const -> bool {
    const ptrdiff_t N = getNumConstTerms();
    invariant(x.size() >= N);
    invariant(y.size() >= N);
...
struct BaseSymbolicComparator : BaseComparator<BaseSymbolicComparator<T>> {
  ...
  constexpr void initNonNegative(math::Alloc<int64_t> auto alloc,
    ...
    Row rowV = math::row(numVar + numConTotal);
    Col colV = math::col(2 * numConTotal);
    ...
    //      A_1'  I  0
    //      S_0  S_1 I]
    static_assert(math::AbstractMatrix<decltype(A.t())>);
  ...
  constexpr void initNonNegative(math::Alloc<int64_t> auto alloc,
    ...
    numVar = ptrdiff_t(A.numCol());
    Row rowV = math::row(numVar + numInEqConTotal);
    Col colV = math::col(2 * numInEqConTotal + numEqCon);
    auto B = getV(rowV, colV);
    std::fill_n(B.begin(), ptrdiff_t(B.numRow()) * ptrdiff_t(B.numCol()), 0);
    B[0, 0] = 1;
  ...
  memoryNeededNonNegative(PtrMatrix<int64_t> A, EmptyMatrix<int64_t>,
    ...
    return memoryNeededImpl(A.numRow(), A.numCol(), Row<>{}, ++numNonNegative);
  }
  [[nodiscard]] inline static constexpr auto
  memoryNeededImpl(Row<> Ar, Col<> Ac, Row<> Er,
                   ptrdiff_t numPos) -> ptrdiff_t {
    // alternative:
  ...
  [[nodiscard]] static constexpr auto
  memoryNeededNonNegative(PtrMatrix<int64_t> A,
                          ptrdiff_t numNonNegative) -> ptrdiff_t {
    return memoryNeededImpl(A.numRow(), A.numCol(), Row<>{}, ++numNonNegative);
  }
  [[nodiscard]] static constexpr auto
  ...
  [[nodiscard]] static constexpr auto memoryNeeded(PtrMatrix<int64_t> A,
                                                   EmptyMatrix<int64_t>,
                                                   bool pos0) -> ptrdiff_t {
    return memoryNeededImpl(A.numRow(), A.numCol(), Row<>{}, pos0);
  }
  [[nodiscard]] static constexpr auto memoryNeeded(PtrMatrix<int64_t> A,
                                                   bool pos0) -> ptrdiff_t {
    return memoryNeededImpl(A.numRow(), A.numCol(), Row<>{}, pos0);
  ...
  constexpr void init(math::Alloc<int64_t> auto alloc, PtrMatrix<int64_t> A,
    ...
    numVar = ptrdiff_t(A.numCol());
    Row<> rowV = math::row(numVar + numCon);
    Col<> colV = math::col(2 * numCon);
    auto B = getV(rowV, colV);
    B << 0;
    B[0, 0] = pos0;
    // V = [A' 0
  ...
  constexpr void init(math::Alloc<int64_t> auto alloc, PtrMatrix<int64_t> A,
    ...
    numVar = ptrdiff_t(A.numCol());
    const ptrdiff_t numEqCon = ptrdiff_t(E.numRow());
    Row rowV = math::row(numVar + numInEqCon);
    Col colV = math::col(2 * numInEqCon + numEqCon);
    auto B = getV(rowV, colV);
    B << 0;
    // V = [A' E' 0
    //      S  0  I]
  ...
  constexpr void initCore(math::Alloc<int64_t> auto alloc) {
    ...
    // after solve, `U` times the value `B` held simplifies it
    ...
    // (2*numInEq + numEq) x R
    auto Ht =
      matrix<int64_t>(alloc, math::row(numColB), math::col(ptrdiff_t(R)));
    Ht << B[_(0, R), _].t();
    solveSystem(Ht, Vt);
    // upper bounded by numVar + numInEq
    // rows/cols, but of rank R
    // smaller based on rank
    getD(R) << Ht.diag().t(); // d.size() == R
  ...
  [[nodiscard]] constexpr auto isEmpty(Arena<> alloc) const -> bool {
    ...
    Row numSlack = math::row(ptrdiff_t(V.numRow()) - numEquations);
    ...
    int64_t lcmD = lcm(d);
    // NOTE: in current vector * Matrix impl, we only read scalars
    // from the vector, and SIMD elements from the matrix. Thus, `/`
    // in the vector won't lead to problems. Eventually, we'll fix
    // the expression templates to not divide by `0` when masking off the
    // remainder.
    ...
    c << (-lcmD * b.t() / d) *
           V[_(numEquations, end), _(begin, numRowTrunc)].t();
    ...
    // where y2 = y2+ - y2-
    auto expandW{matrix<int64_t>(&alloc, numSlack, math::col(dimNS * 2 + 1))};
  ...
  [[nodiscard]] constexpr auto
  greaterEqual(PtrVector<int64_t> query) const -> bool {
    alloc::OwningArena<> alloc;
    return greaterEqual(alloc, query);
  }
  [[nodiscard]] constexpr auto
  greaterEqualFullRank(Arena<> *alloc, PtrVector<int64_t> b) const -> bool {
    auto V = getV();
    if (!allZero(b[_(V.numRow(), end)])) return false;
    ...
    solveSystem(H);
    for (ptrdiff_t i = numEquations; i < H.numRow(); ++i)
      if (int64_t rhs = H[i, oldn])
        if ((rhs > 0) != (H[i, i] > 0)) return false;
    return true;
  }
  [[nodiscard]] constexpr auto
  greaterEqualRankDeficient(Arena<> *alloc,
                            MutPtrVector<int64_t> b) const -> bool {
    auto V = getV();
    auto d = getD();
    Row numSlack = math::row(ptrdiff_t(V.numRow()) - numEquations);
  ...
  [[nodiscard]] constexpr auto
  greaterEqual(Arena<> alloc, PtrVector<int64_t> query) const -> bool {
    auto U = getU();
...
struct LinearSymbolicComparator
  ...
  static constexpr auto construct(PtrMatrix<int64_t> Ap,
                                  bool pos0) -> LinearSymbolicComparator {
    LinearSymbolicComparator cmp;
    cmp.init(alloc::Mallocator<int64_t>{}, Ap, pos0);
    return cmp;
  };
  ...
  constructNonNeg(PtrMatrix<int64_t> Ap, EmptyMatrix<int64_t>,
                  ptrdiff_t numNonNeg) -> LinearSymbolicComparator {
    return constructNonNeg(Ap, numNonNeg);
  };
  static constexpr auto
  constructNonNeg(PtrMatrix<int64_t> Ap,
                  ptrdiff_t numNonNeg) -> LinearSymbolicComparator {
    LinearSymbolicComparator cmp;
    alloc::Mallocator<int64_t> alloc{};
  ...
  static constexpr auto
  constructNonNeg(PtrMatrix<int64_t> Ap, PtrMatrix<int64_t> Ep,
                  ptrdiff_t numNonNeg) -> LinearSymbolicComparator {
    LinearSymbolicComparator cmp;
    alloc::Mallocator<int64_t> alloc{};
...
struct PtrSymbolicComparator
  ...
  constexpr auto getUImpl() -> MutDensePtrMatrix<int64_t> {
    return {mem, DenseDims<>{math::row(rankU), math::col(colU)}};
  }
  // A = V
  // H = A
  // H.truncate(Row());
  ...
  constexpr auto getVImpl() -> MutDensePtrMatrix<int64_t> {
    return {getUImpl().end(), DenseDims<>{numVRows(), math::col(dimV)}};
  }
  // size D
  constexpr auto getDImpl() -> MutPtrVector<int64_t> {
    // d = Ht.diag()
    return {getVImpl().end(), math::length(dimD)};
  }
  [[nodiscard]] constexpr auto getUImpl() const -> DensePtrMatrix<int64_t> {
    return {mem, DenseDims<>{math::row(rankU), math::col(colU)}};
  }
  [[nodiscard]] constexpr auto getVImpl() const -> DensePtrMatrix<int64_t> {
    return {mem + ptrdiff_t(rankU) * colU,
            DenseDims<>{numVRows(), math::col(dimV)}};
  }
  [[nodiscard]] constexpr auto getDImpl() const -> PtrVector<int64_t> {
    return {mem + ptrdiff_t(rankU) * colU + ptrdiff_t(numVRows()) * dimV,
            math::length(dimD)};
  }
  ...
  static constexpr auto
  constructNonNeg(Arena<> *alloc, PtrMatrix<int64_t> Ap, EmptyMatrix<int64_t>,
                  ptrdiff_t numNonNeg) -> PtrSymbolicComparator {
    return constructNonNeg(alloc, Ap, numNonNeg);
  };
  static constexpr auto
  constructNonNeg(Arena<> *alloc, PtrMatrix<int64_t> Ap,
                  ptrdiff_t numNonNeg) -> PtrSymbolicComparator {
    PtrSymbolicComparator cmp(
  ...
  static constexpr auto
  constructNonNeg(Arena<> *alloc, PtrMatrix<int64_t> Ap, PtrMatrix<int64_t> Ep,
                  ptrdiff_t numNonNeg) -> PtrSymbolicComparator {
    PtrSymbolicComparator cmp(
...
private:
  [[nodiscard]] constexpr auto numVRows() const -> Row<> {
    return math::row(ptrdiff_t(dimD ? dimV : rankU));
  }
  ...
  for (ptrdiff_t o = ptrdiff_t(A.numRow()) - 1; o > 0;) {
    for (ptrdiff_t i = o--; i < A.numRow(); ++i) {
      ...
      if (isNeg && C.equalNegative(A[i, _], A[o, _])) {
        ...
        E.resize(math::row(e + 1), math::col(numVar));
...
// NOLINTNEXTLINE(performance-unnecessary-value-param)
constexpr auto
linearNonNegative(alloc::Mallocator<int64_t>, PtrMatrix<int64_t> A,
                  EmptyMatrix<int64_t>,
                  ptrdiff_t numNonNeg) -> LinearSymbolicComparator {
