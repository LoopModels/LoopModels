...
struct LoopTree {
  ...
  [[no_unique_address]] poly::Loop *affineLoop{nullptr};
  ...
  LoopTree(BumpAlloc<> &alloc, llvm::Loop *L, const llvm::SCEV *BT,
    ...
    : loop(L), affineLoop{poly::Loop::construct(alloc, L, BT, SE)} {
    paths.push_back(std::move(pth));
  }

  LoopTree(BumpAlloc<> &alloc, llvm::Loop *L, NotNull<poly::Loop> aln,
