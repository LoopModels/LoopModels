...
//   static constexpr void assertCanonical() {}
  ...
  constexpr auto getNumCons() const -> unsigned { return numConstraints; }
  constexpr auto getNumVars() const -> unsigned { return numVars; }
...
// search for entering variable
  ...
  static auto positiveVariables(BumpAlloc<> &alloc, PtrMatrix<int64_t> A,
    ...
    PtrSimplex simplex{
      PtrSimplex::create(alloc, numCon, numVar, numCon, varCap, numSlack)};
    // construct:
    // [ I A
    //   0 B ]
  ...
  void pruneBounds(BumpAlloc<> &alloc) {
    ...
    PtrSimplex simplex{PtrSimplex::create(
      alloc, tableau.numConstraints, tableau.numVars,
      tableau.constraintCapacity, tableau.varCapacity, numSlack)};
  ...
  static constexpr auto create(BumpAlloc<> &alloc, unsigned conCap,
                               unsigned varCap, unsigned numSlack)
    -> PtrSimplex {
    Tableau tableau = Tableau::create(alloc, conCap, varCap);
    return PtrSimplex{tab, slack};
  }
  static constexpr auto create(BumpAlloc<> &alloc, unsigned numCon,
                               unsigned numVar, unsigned conCap,
                               unsigned varCap, unsigned numSlack)
  ...
  constexpr auto operator<<(const PtrSimplex &other) -> PtrSimplex & {
    ...
    numSlackVar = other.numSlackVar;
    return *this;
  };

  // struct Simplex : PtrSimplex {

  // };

  /// The goal here:
  /// this Simplex struct will orchestrate search through the solution space
  /// it will add constraints as it goes, e.g. corresponding to desired
  /// properties or as we move up loop levels to maintain independence from
  /// previous ones.
  struct Simplex {
    ...
    constexpr auto addConstraint() -> MutPtrVector<int64_t> {
      tableau.resize(StridedDims{tableau.numRow() + 1, tableau.numCol(),
                                 tableau.rowStride()});
      tableau(last, _) << 0;
    ...
    auto removeAugmentVars(llvm::ArrayRef<unsigned> augmentVars,
                           ptrdiff_t numVar) -> bool {
      // TODO: try to avoid reallocating, via reserving enough ahead of time
