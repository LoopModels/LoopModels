...
struct DependencePolyhedra {
    ...
    DependencePolyhedra(AffineLoopNest &aln0, AffineLoopNest &aln1,
        ...
        const llvm::SmallVector<std::pair<size_t, size_t>, 4> &dims =
            maybeDims.getValue();
            ...
            if (!isZero(aln1.b[i])) {
                for (auto &t : aln1.b[i]) {
                    if (auto c = t.getCompileTimeConstant()) {
                        ...
                        size_t j = constantTerms[t.exponent];
                        As(iOff + i, j + numLoopConstraints) =
                            -t.coefficient;
                        As(iOff + i, j + numLoopConstraints) =
                            t.coefficient;
                    }
    ...
    static llvm::Optional<llvm::SmallVector<std::pair<size_t, size_t>, 4>>
    matchingStrideConstraintPairs(const AffineLoopNest &aln0,
        ...
        if (ar0.stridesMatchAllConstant(ar1)) {
            llvm::SmallVector<std::pair<size_t, size_t>, 4> dims;
