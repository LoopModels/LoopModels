...
#include <utility>
  ...
  public:
    CPURegisterFile(llvm::LLVMContext &C,
                    const llvm::TargetTransformInfo &TTI) {
  ...
  inline auto searchLoopIndependentUsers(IR::Dependencies deps, IR::Loop * L,
    ...
    if (a) {
      if (ret.summary.notIndexedByLoop == a)
        ret.summary.notIndexedByLoop =
          llvm::cast_or_null<IR::Addr>(a->getNext());
    }
    I->removeFromList();
  ...
  // NOLINTNEXTLINE(misc-no-recursion)
  inline auto visitLoopDependent(IR::Dependencies deps, IR::Loop * L,
                                 IR::Node * N, uint8_t depth, IR::Node * body)
    ->IR::Node * {
  ...
  inline auto topologicalSort(IR::Dependencies deps, IR::Loop * root,
    ...
    for (IR::Node *N : C->nodes())
      summary =
        searchLoopIndependentUsers(deps, root, N, depth, summary).summary;
  ...
  public:
    IROptimizer(IR::Dependencies deps, IR::Node *nodes, Arena<> *lalloc,
                Arena<> *salloc)
      : deps{deps}, nodes{nodes},
        loopDeps{loopDepSats(alloc, deps, nodes->getOptimizationResult())},
